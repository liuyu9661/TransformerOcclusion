...
def create_dataset(dataset_folder,dataset_name,val_size,gt,horizon,delim="\t",train=True,eval=False,verbose=False):
       #val_size =0, gt=obs_len, horizon= pre_obs
        if train==True:
        ...
        for i_dt, dt in enumerate(datasets_list):
            ...
            #os.system("PAUSE")
            inp,out,info=get_strided_data_clust(raw_data,gt,horizon,step)  #inp[seqId, obsLen, pos+speed], out[seqId, preLen, pos+speed], info{mean, std, start, frames, pedId}

            dt_frames=info['frames']                                #frames[seqId, StartFrameId:EndFrameId]
            dt_seq_start=info['seq_start']                          #seq_start[seqId, StartFrameId]
            dt_dataset=np.array([i_dt]).repeat(inp.shape[0])        #[datesetId], len = len(frames.shape(0))
            dt_peds=info['peds']                                    #peds[pedID]
        ...
        # For the dataset ETH
        data['src'] = np.concatenate(data_src, 0)                 #inp[seqId, obsLen, pos+speed],  info{mean, std, start, frames, pedId}
        data['trg'] = np.concatenate(data_trg, 0)                 #out[seqId, preLen, pos+speed],
        data['seq_start'] = np.concatenate(data_seq_start, 0)     #seq_start[seqId, StartFrameId], [seqId, 1]
        data['frames'] = np.concatenate(data_frames, 0)           #frames[seqId, StartFrameId:EndFrameId]
        data['dataset'] = np.concatenate(data_dt, 0)              #[datesetId], len = len(seqId)
        data['peds'] = np.concatenate(data_peds, 0)               #peds[pedID], [len(frames.shape(0))]
...
def get_strided_data_clust(dt, gt_size, horizon, step):
    ...
    frame=[]
    ped_ids=[]
    for p in ped: #each ped
        for i in range(1+(raw_data[raw_data.ped == p].shape[0] - gt_size - horizon) // step):  # generate seq for each ped
            frame.append(dt[dt.ped == p].iloc[i * step:i * step + gt_size + horizon, [0]].values.squeeze()) #record each seq frame ID (frist col [0])
            # print("%i,%i,%i" % (i * 4, i * 4 + gt_size, i * 4 + gt_size + horizon))
            inp_te.append(raw_data[raw_data.ped == p].iloc[i * step:i * step + gt_size + horizon, 2:4].values)#record each seq date (second~fourth col 2:4)
            ped_ids.append(p)

    frames=np.stack(frame)                     #[seqId, seqstartID: seqendID]  ,    [seqId, obslen+prelen]
    inp_te_np = np.stack(inp_te)               #[seqId, seqLen, pos], [seqId, obslen+prelen, 2]
    ped_ids=np.stack(ped_ids)                  #[pedID] pedID for seqId

    # print(np.shape(frames))
    # print(np.shape(inp_te_np))
    # print(np.shape(ped_ids))

    #os.system("PAUSE")
    #inp_relative_pos= inp_te_np-inp_te_np[:,:1,:]
    # print("="*10)
    inp_speed = np.concatenate((np.zeros((inp_te_np.shape[0],1,2)),inp_te_np[:,1:,0:2] - inp_te_np[:, :-1, 0:2]),1)  #[seqId, 20, speed], speed=2, speed at first frame is 0

    # print(np.shape(inp_speed))
    #os.system("PAUSE")
    #inp_accel = np.concatenate((np.zeros((inp_te_np.shape[0],1,2)),inp_speed[:,1:,0:2] - inp_speed[:, :-1, 0:2]),1)
    #inp_std = inp_no_start.std(axis=(0, 1))
    #inp_mean = inp_no_start.mean(axis=(0, 1))
    ...
    #vis=inp_te_np[:,1:,2:4]/np.linalg.norm(inp_te_np[:,1:,2:4],2,axis=2)[:,:,np.newaxis]
    #inp_norm=np.concatenate((inp_norm,vis),2)
    inp_norm=np.concatenate((inp_te_np,inp_speed),2)   #[seqId, seqlen, pos+speed], [seqId, 20, 4]
    # print(np.shape(inp_norm))
    # os.system("PAUSE")
    inp_mean=np.zeros(4)
    inp_std=np.ones(4)

    return inp_norm[:,:gt_size],inp_norm[:,gt_size:],{'mean': inp_mean, 'std': inp_std, 'seq_start': inp_te_np[:, 0:1, :].copy(),'frames':frames,'peds':ped_ids} #frames [seqId, startSeqid:endSeqid]
